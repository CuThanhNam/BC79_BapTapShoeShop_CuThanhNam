  ...
  export default function RenderWithMap() {
    let renderListMovie = () => {
        return listMovie.map((item, index) => {
            ...
            // </div>
            <ItemMovie tenPhim={tenPhim} hinhAnh={hinhAnh} />
            );
          });
          // dùng map để duyệt listMovie và render ra tên phim
      };
      return (
        <div>
          <h2>RenderWithMap</h2>
          <div className="grid gap-10 grid-cols-6">{renderListMovie()}</div>
        </div>
      );
  }

// map ~ duyệt mảng và trả về một mảng mới => bắt buộc phải return
